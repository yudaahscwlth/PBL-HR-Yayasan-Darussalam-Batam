...
function ActionMenuPortal({
  ...
  const menu = (
    ...
    >
      <Link
        href={`/kepala-departemen/profile/${userId}`}
      ...
      <Link
        href={`/kepala-departemen/absensi/${userId}`}
        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-sky-800 flex items-center gap-2"
      >
      ...
      <Link
        href={`/kepala-departemen/cuti/${userId}`}
        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-sky-800 flex items-center gap-2"
      >
      ...
      <Link
        href={`/kepala-departemen/evaluasi/${userId}`}
        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-sky-800 flex items-center gap-2"
      >
...
export default function KepalaDepartemenKelolaPegawaiPage() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/departemen/${currentDeptId}/members`,
          {
      ...
      <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans text-gray-800">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-800 mb-6">Daftar Pegawai Departemen</h2>
                      ...
                      <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-1 rounded text-xs font-medium capitalize
                            ${user.profile_pekerjaan?.status === 'aktif' ? 'bg-emerald-100 text-emerald-700' :
                              user.profile_pekerjaan?.status === 'tetap' ? 'bg-green-100 text-green-700' :
                              user.profile_pekerjaan?.status === 'kontrak' ? 'bg-blue-100 text-blue-700' :
                              user.profile_pekerjaan?.status === 'honorer' ? 'bg-yellow-100 text-yellow-700' :
                              user.profile_pekerjaan?.status === 'magang' ? 'bg-purple-100 text-purple-700' :
                              user.profile_pekerjaan?.status === 'nonaktif' ? 'bg-red-100 text-red-700' :
                              user.profile_pekerjaan?.status === 'pensiun' ? 'bg-gray-100 text-gray-700' :
                              user.profile_pekerjaan?.status === 'cuti' ? 'bg-orange-100 text-orange-700' :
                              user.profile_pekerjaan?.status === 'skorsing' ? 'bg-rose-100 text-rose-700' :
                              'bg-slate-100 text-slate-700'}`}>
                            {user.profile_pekerjaan?.status || "Belum Set"}
                          </span>
                        </td>
