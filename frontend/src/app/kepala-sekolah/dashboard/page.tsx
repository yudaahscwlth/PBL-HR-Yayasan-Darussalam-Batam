...
export default function KepalaSekolahDashboard() {
          ...
          (error) => {
            ...
            switch (error.code) {
              case error.PERMISSION_DENIED:
                errorMessage =
                  "Akses lokasi ditolak. Silakan izinkan akses lokasi untuk melakukan absensi.";
                break;
              ...
              case error.TIMEOUT:
                errorMessage =
                  "Timeout saat mengambil lokasi. Coba pindah ke area dengan sinyal GPS yang lebih baik.";
                break;
            }
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const locationWithAccuracy = location as {
        latitude: number;
        longitude: number;
        accuracy?: number;
      };
      if (
        locationWithAccuracy.accuracy &&
        locationWithAccuracy.accuracy > gpsTolerance
      ) {
      ...
      if (response.success) {
        ...
        if (response.message?.includes("luar area kerja")) {
          ...
          toast.error(
            response.message ||
              "❌ Gagal melakukan check in. Silakan coba lagi."
          );
        }
      }
    } catch (error: unknown) {
      ...
      if (
        err.message &&
        (err.message.includes("lokasi") || err.message.includes("GPS"))
      ) {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.message?.includes("luar area kerja")) {
          ...
          toast.error(
            response.message ||
              "❌ Gagal melakukan check out. Silakan coba lagi."
          );
        }
      }
    } catch (error: unknown) {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (statusLower.includes("hadir") && !statusLower.includes("tidak")) {
      ...
      statusLower.includes("tidak hadir") ||
        ...
        <div className="flex items-center justify-between mb-1">
          <div className="flex-1">
            ...
            <h1 className="text-xl font-bold">
              {user?.profile_pribadi?.nama_lengkap || user?.email}
            </h1>
          ...
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center overflow-hidden border-2 border-white shadow-md">
            {user?.profile_pribadi?.foto ? (
              <img
                ...
                }/storage/${user.profile_pribadi.foto}`}
                alt={user?.profile_pribadi?.nama_lengkap || "Profile"}
                className="w-full h-full object-cover"
              />
            ) : (
              <svg
                className="w-6 h-6 text-[#1e4d8b]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="flex gap-3">
            ...
            >
              {isLoading ? (
                <div className="flex items-center">
                  <svg
                    className="animate-spin h-5 w-5 text-[#1e4d8b] mr-2"
                  ...
                  <span className="text-[#1e4d8b] font-semibold text-sm">
                    Loading...
                  </span>
                </div>
              ) : attendanceStatus.hasCheckedIn ? (
      ...
      <div className="px-5 mt-4">
        <div className="bg-white rounded-2xl shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              <svg
                className="w-6 h-6 text-[#1e4d8b] mr-3"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                ...
                <div className="space-y-2">
                  {dashboardData.recentLeaves
                    ...
                    .map((leave: any, index: number) => {
                      const leaveType =
                        leave.tipe_cuti || leave.jenis_cuti || "Cuti";
                      const status =
                        leave.status_pengajuan || leave.status || "pending";
                      const statusLower = status.toLowerCase();
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Masuk: {formatTime(record.check_in)}, Pulang:{" "}
