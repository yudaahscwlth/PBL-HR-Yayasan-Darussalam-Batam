...
export default function TenagaPendidikLogAbsensiPage() {
    ...
    useEffect(() => {
        ...
        const loadLogData = async () => {
            try {
                setIsLoading(true);
                // Using the same endpoint since it seems to be shared/accessible
                const response = await apiClient.attendance.getLog(attendanceId);

                if (response.success && response.data) {
                    setLogData(response.data as AttendanceLogData);
                } else {
                    // If authorized but no data or success false
                    console.error(response.message || "Gagal memuat log absensi");
                    // Don't alert immediately on SSR/mount mismatch, let the UI handle empty state
                }
            } catch (error: any) {
